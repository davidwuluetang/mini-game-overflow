...
class Dropdown:
    ...
    def draw(self, window):
        ...
        window.blit(text, (self.x + 50, self.y + 5))
...
class Board:
    ...
    def undoMove(self, oldBoard, numOfHumanPlayer):
        self.set(oldBoard)
        if numOfHumanPlayer == 2:
            self.turn -= 1
        elif numOfHumanPlayer == 1:
            self.turn -= 2
...
class Button:
    ...
    def draw(self, window, textColor=(0,0,0), bgColor=None):
        pygame.draw.rect(window, BLACK, (self.x, self.y, self.width, self.height), 2)
        font = pygame.font.Font(None, 36)
        text = font.render(self.string, 1, textColor, bgColor)
        window.blit(text, (self.x + 20, self.y + 13))
...
p1spritesheet = pygame.image.load('image/blue.png')
p2spritesheet = pygame.image.load('image/orange.png')
p1_sprites = []
p2_sprites = []


player_id = [1 , -1]
# AI level - 3 levels. Each number in index 0 represent branch factor of game tree,
# and each number in index 1 represent tree height.
AI_levels = [[2, 4],
             [2, 11],
             [2, 17]]
...
AI_level_dropdown = Dropdown(900, 170, 120, 50, ['1', '2', '3']) # Add AI level adjustment
undoBtn = Button(900, 300, 170, 50, "Undo Move")
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            AI_level_dropdown.handle_event(event) # For AI level
            choice[0] = player1_dropdown.get_choice()
            choice[1] = player2_dropdown.get_choice()
            choice[2] = AI_level_dropdown.get_choice() # Change AI level
            current_AI_level = AI_levels[int(choice[2])]
            if event.type == pygame.MOUSEBUTTONDOWN:
                x,y = event.pos
                row = y - Y_OFFSET
                col = x - X_OFFSET    
                grid_row, grid_col = row // CELL_SIZE, col // CELL_SIZE
            if event.type == pygame.KEYDOWN: # Handle 'R' key for game reset
                if event.key == pygame.K_r and has_winner is True:
                    board.reset()
                    has_winner = False
                    current_player = 1
                    while board_stacks.is_empty() is not True:
                        board_stacks.pop()
            if undoBtn.onClick(event) and bothPlayerIsAI(choice) is not True and board_stacks.is_empty() is not True:
                numOfHumanPlayer = 0
                if choice[0] == 0:
                    numOfHumanPlayer += 1
                if choice[1] == 0:
                    numOfHumanPlayer += 1
                for i in range(2 - numOfHumanPlayer):
                    board_stacks.pop()
                board.undoMove(board_stacks.pop(), numOfHumanPlayer)
                if numOfHumanPlayer == 2:
    ...
    if not has_winner:
        if overflowing:
            ...
            if choice[current_player] == 1:
                (grid_row,grid_col) = bots[current_player].get_play(board.get_board(), current_AI_level)
            ...
            if make_move:
                board_stacks.push(board.get_board())
    ...
    font = pygame.font.Font(None, 36)
    text = font.render('AI Level', 1, BLACK)
    window.blit(text, (780, 180))
    frame = (frame + 0.5) % 8
    player1_dropdown.draw(window)
    player2_dropdown.draw(window)
    AI_level_dropdown.draw(window)
    if bothPlayerIsAI(choice) is not True: # If any player is not AI
        undoBtn.draw(window, BLACK, (255,33,33)) # draw 'Undo Move' button
    ...
    if not has_winner:  
        ...
        resetText = font.render("Press \'R\' to reset game", True, (255,33,33)) # Red color
        window.blit(text, (300, 250))
        window.blit(resetText, (400, 350))
